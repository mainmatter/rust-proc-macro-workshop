...
hand with a `syn` `Parse` impl, but you already met the better tool in chapter 3:
...
- **[`NestedMeta::parse_meta_list`](https://docs.rs/darling/latest/darling/ast/enum.NestedMeta.html)**
  ...
  error type.
- **`from_list`** is the `FromMeta` entry point, mirroring `from_derive_input` from chapter 3. It
